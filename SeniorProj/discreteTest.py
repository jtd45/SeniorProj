...
if __name__=='__main__':
	try:
		...
		# while i<6:
				...
				# image=cv2.resize(image,(832,624))
				# if i<7:
					# port.write_Serial("70")
				# elif i==7:
					# port.write_Serial("0")
					# print("center")
					# time.sleep(4)
					# port.write_Serial("f0")
				# else:
					# port.write_Serial("f0")
				# print("hi ",i)
		...
		# while(video.isOpened()):
			...
			# if state==0:
				# filt,combo,diff,noted=diffFilter(image,images[i])
				# images2.append(image)
				# time.sleep(1)
				# i+=1
			# elif state==1:
				# images3.append(image)
				# filt,combo,diff,noted,t=diffFilter2(image,images[i],images2[i])
				# cv2.imshow("thresh",t)
				# k=cv2.waitKey(10)&0xFF
				# if k==27:
					# break
			...
			# else:
				# if count<1:
					# port.write_Serial("0")
					# time.sleep(1)
					# port.write_Serial("10")
					# i=1
					# count=5
				# filt,t=diffFilter3(image,images[i],images2[i],images3[i])#,images3[i])
				# cv2.imshow("thresh",t)
				# k=cv2.waitKey(10)&0xFF
				# if k==27:
					# break
			...
			# #cv2.imshow("deff",diff)
			# #cv2.imshow("not",noted)
			# k=cv2.waitKey(10)&0xFF
			# if k==27:
				# break
			# if cv2.getWindowProperty("title",0)==-1:
